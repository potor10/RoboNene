/**
 * @fileoverview Command Data & Constants Related to the /about command
 * These constants are used to dynamically generate a slash command on discord.
 * This File also contains the constants values used exclusively in the /about command.
 * @author Potor10
 */

module.exports = {
  'INFO': {
    'name': 'about',
    'utilization': '/about',
    'description': 'Learn about Nene Robo',
    'ephemeral': true
  },

  'CONSTANTS': {
    'INTERACTION_TIME': 60000,

    'LEFT': '⬅️',
    'RIGHT': '➡️',

    'PROGRAMMERS': [
      'Potor10#3237',
      'Ult#0001',
      'Redside#1337',
      'Yuu#6883',
      'Maid'
    ],

    'DESIGNERS': [
      'Reinhäla',
      'Potor10#3237'
    ],

    'TESTERS': [
      'Reinhäla',
      'i like potor and airi',
      'uhh',
      'Sela',
      'PbSky',
      'chi',
      'morgz',
      'tiramiiz#2456',
      'cecil#1309',
      'Plutone'
    ],

    'GAME_DATA_LINK': 'https://github.com/Sekai-World/sekai-master-db-en-diff/blob/main/events.json',
    'ESTIMATION_LINK': 'https://docs.google.com/document/d/1137qbA0_qWOHJGhaYfxpgftqhsJvW4qo3QBTcSqnZo8/edit#heading=h.pkcfws4d84gx',
    'RANKING_DATA_LINK': 'https://api.sekai.best/docs',

    'ABOUT_DISCORD': 'Nene Robo logs the usage of her commands to prevent abuse. ' +
      'Outside of the use of her commands, Nene Robo does not collect any user data, chat logs, or server information.\n\n' +
      'Nene Robo will store information on Discord ID, Project Sekai ID, if you choose to link accounts. ' +
      'Nene Robo will store the Channel ID and Server ID if you enable tracking. ' +
      'No information will be saved if you choose to disable these features.\n\n' +
      'Nene Robo’s code is open-source, you can check it out at [github](https://github.com/potor10/RoboNene).',

    'ABOUT_SEKAI': 'Linking accounts with Nene Robo is non-invasive. ' + 
      'Nene Robo will not be able to access or affect your account in any way through linking. \n\n' +
      'Nene Robo will never ask for, nor ever need access to your Project Sekai account to function. ' +
      'Linking accounts exists solely to prevent misuse of certain commands and provide ease of integration between a user and their Project Sekai account.',

    'ABOUT_CONTRIBUTE': 'You can contribute to Nene Robo by opening a pull request at [github](https://github.com/potor10/RoboNene/pulls).',

    'ABOUT_TECH': 'Nene Robo utilizes discord.js to serve her content. ' + 
      'She saves tiering history and linked account information through use of sqlite3 databases. ' +
      'She uses regression-js to fit data into a linear model and provide estimates.',

    'ABOUT_LICENSE': 'MIT'
  }
}